...
data2 = pickle.load(open('/home/jjs/git/JScar/bag/test_pickle.p', 'rb'))
#data3 = pickle.load(open('./data/recovery.p', 'rb'))
# data4 = pickle.load(open('./data/map2_4laps.p', 'rb'))

images = data['features']
measurements = data['labels']
images_2 = data2['features']
# images_1 = data3['features']
# images_2 = data4['features']

measurements_2 = data2['labels']
# measurements_1 = data3['labels']
# measurements_2 = data4['labels']
#
...
model.vision_model.compile(optimizer=Adam(lr=0.00003), loss=[mse_steer_angle])

checkpoint = ModelCheckpoint('./model/checkpoints/weights.{epoch:02d}-{val_loss:.3f}.h5', save_weights_only=True)
model.vision_model.fit(images, measurements, batch_size=128, nb_epoch=1, callbacks=[checkpoint],
